...
- Media albums: delivered instantly via `get_media_group` — an anchored `getChatHistory` slice; a Telegram album is atomic on the server, so one fetch returns every part (DNA-1124). The per-`[chat_id, media_album_id]` debounce buffer (debounce window + hard hold deadline) remains as the fallback when the fetch fails. Delivered-album tracking is id-aware: one fetch per album, but a late tail absent from the fetched batch still goes through (downstream tail-merge self-healing depends on it). `spec/extension/custom_update_handler_spec.rb` documents the exact semantics.
